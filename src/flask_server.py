...
if __name__ == "__main__":
    ...
    # ChatGPT-compatible API endpoints
    @app.route('/v1/chat/completions', methods=['POST'])
    def chat_completions():
        global global_text, global_state, system_prompt, received_messages
        global is_blocking

        # If the server is in a blocking state, return a specific response.
        if is_blocking or global_state==0:
            return jsonify({'error': {'message': 'Server is busy', 'type': 'server_error', 'code': 503}}), 503
        ...
        try:
            ...
            rkllm_model.rkllm_infer_params.mode = RKLLMInferMode.RKLLM_INFER_GENERATE
            ...
            if not stream:
                ...
                # Process messages to form the prompt
                input_prompt = ""
                ...
                # Add a prompt to indicate the assistant should respond
                input_prompt += "\\nAssistant:"

                # Run the model
                rkllm_output = ""
                model_thread = threading.Thread(target=rkllm_model.run, args=('assistant', False, input_prompt))
                model_thread.start()
                ...
                # Format response in ChatGPT format
                response = {
                    ...
                    "usage": {
                        ...
                        "completion_tokens": len(rkllm_output),
                        "total_tokens": len(input_prompt) + len(rkllm_output)
                    }
                }
                    ...
                    while not model_thread_finished:
                        while len(global_text) > 0:
                            ...
                            response = {
                                ...
                                "created": int(time.time()),
                                "model": model,
                                "system_fingerprint": "rkllm-server",
                                "choices": [{
                                    "index": 0,
                                    "delta": {"content": token},
                                    "logprobs": None,
    ...
    def list_models():
        ...
        models_list = {
            ...
            "data": [
                {
                    "id": "deepseek-r1-distill-qwen-1.5b-fp16-rk3576",
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "organization-owner"
                },
                {
                    "id": "deepseek-r1-distill-qwen-1.5b-w4a16-rk3576",
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "organization-owner"
                },
                {
                    "id": "deepseek-r1-distill-qwen-7b-w4a16-rk3576",
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "organization-owner"
                },
                {
                    "id": "deepseek-r1-distill-owen-7b-w4a16-g128-rk3576",
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "organization-owner"
                }
