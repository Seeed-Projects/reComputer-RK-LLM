...
# 拷贝脚本与工具
COPY ./src/fix_freq_rk3576.sh /app/
...
# 启动命令：增加自动频率锁定（针对 rk3576 优化性能）
CMD ["sh", "-c", "/app/fix_freq_rk3576.sh && python3 /app/main.py --rkllm_model_path ${RKLLM_MODEL_PATH} --target_platform rk3576"]